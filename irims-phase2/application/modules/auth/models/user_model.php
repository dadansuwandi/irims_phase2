...
class User_model extends MY_Model 
{
	...
	protected $grade_table = 'ac_role_grade_maping';
	protected $user_role_table = 'ac_user_role_maping';
	private $ci;
	 ...
	 * @return object grade
	 */
	function get_role_by_grade($employeeGrade)
	{	

		$this->db->select($this->grade_table . '.*, ' . $this->role_table . '.name AS role_name')
				->join($this->role_table, $this->role_table . '.id = ' . $this->grade_table . '.RoleID', 'inner');
		$query = $this->db->get_where($this->grade_table, array($this->grade_table . '.grade' => $employeeGrade));
		if ($query->num_rows() > 0)
			return $query->row();
		else
			return FALSE;
	...
	{	
		$this->db->select($this->user_role_table . '.*, ' . $this->role_table . '.name AS role_name')
				->join($this->role_table, $this->role_table . '.id = ' . $this->user_role_table . '.RoleID', 'inner');
		$query = $this->db->get_where($this->user_role_table, array($this->user_role_table . '.UserName' => $username));
		if ($query->num_rows() > 0)
			return $query;
		else
			return FALSE;
