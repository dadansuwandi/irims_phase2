...
class worksheet extends Admin_Controller {
    ...
    function header() {    
       ...
       // var_dump($delete_worksheet_header);die;
		    $this->template->build('worksheet_header');
    }
    function headerrender() {    
    
      // var_dump($delete_worksheet_header);die;
       $this->template->build('worksheet_header_render');
   }
    function masterworksheet() {
		  $this->template               
                ->build('worksheet_master');
    }

    function get_worksheetHeader(){
       $get_worksheetHeader_list = $this->worksheet_model->get_worksheetHeader_list();
    ...
    function insert_WorkHeader(){
      ...
      if($_POST['IsActive'] == 1){
        $update_worksheet_header_isActive = $this->worksheet_model->update_worksheet_header_isActive(0);
      }     


       $insert_worksheet_header = $this->worksheet_model->insert_worksheet_header( $ls);
    ...
    function delete_workSheetheader(){
      ...
      $WorksheetHeaderID  = $_POST['WorksheetHeaderID'];
       ...
       $delete_worksheet_header = $this->worksheet_model->delete_worksheet_header($WorksheetHeaderID);
    ...
    function update_worksheetheader(){
      ...
      //var_dump($Grade);die;		
       $update_worksheet_header = $this->worksheet_model->update_worksheet_header($WorksheetHeaderID,$ls);

      if($_POST['IsActive'] == 1){
        $update_worksheet_header_isActive = $this->worksheet_model->update_worksheet_header_isActive($WorksheetHeaderID);
      }
    ...
    function get_worksheetHeader_by_id(){
      ...
      $WorksheetHeaderID  = $_POST['WorksheetHeaderID'];
      $get_worksheetHeader_by_id = $this->worksheet_model->get_worksheetHeader_by_id($WorksheetHeaderID);
   ...
   function get_worksheet_form_active_by_WorkSheetFlowID(){
          ...
          $get_worksheet_form_active = $this->worksheet_model->get_worksheet_form_active($WorkSheetFlowID);
     ...
     function get_worksheetmaster_By_WorkSheetFlowID()
     {
      ...
      $get_worksheetmaster_By_WorkSheetFlowID = $this->worksheet_model->get_worksheetmaster_By_WorkSheetFlowID($WorksheetHeaderID,$WorkSheetFlowID);
     ...
     function update_worksheet_form()
     {
      ...
      $update_worksheet_form = $this->worksheet_model->update_worksheet_form($WorksheetFormID,$ls);
