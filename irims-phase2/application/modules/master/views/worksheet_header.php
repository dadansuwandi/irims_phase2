...
<script> 
	...
	function loadData(){
		...
		var get_worksheetHeader = InvokeCodeBehind("worksheet","get_worksheetHeader", null);
		debugger;
	...
	function InsertData()
	{
		debugger;
			...
			var SdateObj = moment(SDate, 'MM-DD-YYYY');
			var Sdate_ = SdateObj.format('YYYY-MM-DD');

			var EDate = $('#EndDate').val();
			var EdateObj = moment(EDate, 'MM-DD-YYYY');
			var Edate_ = EdateObj.format('YYYY-MM-DD');

			debugger;
		
	    	var formData = new FormData();
			formData.append("WorkSheetName", $('#WorksheetName').val());
			formData.append("StartDate",  Sdate_);
			formData.append("EndDate",  Edate_);
			formData.append("IsActive",  ($("#IsActive").prop("checked") == true) ? '1' : '0');
			formData.append("CreatedBy", 'admin' );
			formData.append("CreatedDate", '2023-12-23');
			var insert_WorkHeader = InvokeCodeBehind("worksheet","insert_WorkHeader", formData);
			debugger;
		...
		function UpdateData(){
			debugger;
				...
				formData.append("CreatedBy", 'admin' );
				formData.append("CreatedDate", '2023-12-23');
				var update_worksheetheader = InvokeCodeBehind("worksheet","update_worksheetheader", formData);
				debugger;
		...
		function DeleteData(ID)
		{
			...
			.yes(() => { 
				...
				var delete_workSheetheader = InvokeCodeBehind("worksheet","delete_workSheetheader", formData);
				debugger;
